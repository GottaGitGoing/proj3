...
public:
    ...
    // find larget bucket and returns node count
    unsigned int findLargestBucket() const;
...
private:
    ...
    unsigned int sz; // Size used for knowing where to add the items
    unsigned int cap; // Cap to know max bucket amount
